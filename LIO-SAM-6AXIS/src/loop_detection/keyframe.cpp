...
KeyFrame::KeyFrame(double _time_stamp, int _index, const cv::Mat &_image_intensity,
...
void KeyFrame::computeWindowOrbPoint() {
    if (USE_ORB) {
        // ORB features
        vector<uchar> status;
        cv::Ptr<cv::ORB> detector = cv::ORB::create(NUM_ORB_FEATURES * 10, 1.2f, 8, 1);
...
void KeyFrame::computeBoWPoint() {
    ...
    // ROS_INFO("ORB FEATURE SIZE: %d", keypoints.size());
...
bool KeyFrame::findConnection(KeyFrame *old_kf) {
    ...
    pubMatchedImages(&pub_bow_img, time_stamp, false, 1, cv::Scalar(0, 255, 0), index, old_kf->index, thumbnail,
                     old_kf->thumbnail, search_orb_point_2d_uv, old_kf->search_orb_point_2d_uv);
    ...
    if (USE_ORB) {
        ...
        cv::BFMatcher matcher = cv::BFMatcher(
                cv::NORM_HAMMING); // https://docs.opencv.org/3.3.1/d3/da1/classcv_1_1BFMatcher.html
        ...
        if ((int) good_matches.size() > MIN_LOOP_FEATURE_NUM) {
            ...
            pubMatchedImages(&pub_prepnp_img, time_stamp, true, 5, cv::Scalar(0, 255, 0), index, old_kf->index,
                             thumbnail, old_kf->thumbnail, matched_2d_cur, matched_2d_old);
            ...
            if ((int) matched_2d_cur.size() > MIN_LOOP_FEATURE_NUM &&
                distributionValidation(matched_2d_cur, matched_2d_old)) {
                pubMatchedImages(&pub_match_img, time_stamp, true, 5, cv::Scalar(0, 255, 0), index, old_kf->index,
                                 thumbnail, old_kf->thumbnail, matched_2d_cur, matched_2d_old);
                return true;
            }
        }
    ...
    if (USE_BRIEF) {
        ...
        searchByBRIEFDes(matched_2d_old, matched_2d_old_norm, status, brief_window_descriptors,
                         old_kf->search_brief_descriptors, old_kf->search_brief_point_2d_uv,
                         old_kf->search_brief_point_2d_norm);
        reduceVector(matched_3d, status);
        reduceVector(matched_2d_cur, status);
        reduceVector(matched_2d_old, status);
        reduceVector(matched_2d_old_norm, status);

        pubMatchedImages(&pub_prepnp_img, time_stamp, true, 5, cv::Scalar(0, 255, 0), index, old_kf->index, thumbnail,
                         old_kf->thumbnail, matched_2d_cur, matched_2d_old);
...
void KeyFrame::PnPRANSAC(const vector<cv::Point2f> &matched_2d_old_norm,
    ...
    try {
        ...
        ROS_ERROR("PNP METHOD SIZE: %d, %d", matched_3d.size(), matched_2d_old_norm.size());
    }
...
bool KeyFrame::distributionValidation(const vector<cv::Point2f> &new_point_2d_uv,
...
void KeyFrame::freeMemory() {
    ...
    brief_point_3d.clear();
    brief_point_3d.shrink_to_fit();
    brief_point_2d_uv.clear();
    brief_point_2d_uv.shrink_to_fit();
    brief_point_2d_norm.clear();
    brief_point_2d_norm.shrink_to_fit();
    brief_window_keypoints.clear();
    brief_window_keypoints.shrink_to_fit();
    brief_window_descriptors.clear();
    brief_window_descriptors.shrink_to_fit();

    search_brief_point_3d.clear();
    search_brief_point_3d.shrink_to_fit();
    search_brief_keypoints.clear();
    search_brief_keypoints.shrink_to_fit();

    orb_point_3d.clear();
    orb_point_3d.shrink_to_fit();
    orb_point_2d_uv.clear();
    orb_point_2d_uv.shrink_to_fit();
    orb_point_2d_norm.clear();
    orb_point_2d_norm.shrink_to_fit();
    orb_window_keypoints.clear();
    orb_window_keypoints.shrink_to_fit();
    orb_window_descriptors.release();

    search_orb_point_3d.clear();
    search_orb_point_3d.shrink_to_fit();
    search_orb_keypoints.clear();
    search_orb_keypoints.shrink_to_fit();

    bow_descriptors.clear();
    bow_descriptors.shrink_to_fit();
}