...
public:
    ...
    void cloud_handler(const sensor_msgs::PointCloud2ConstPtr &cloud_msg) {
        ...
        if (laser_cloud->size() % IMAGE_HEIGHT * IMAGE_WIDTH != 0) {
            ROS_ERROR("MAKE SURE YOUR POINTS SIZE IS EQUAL TO IMAGE_HEIGHT * IMAGE_WIDTH %d, %d", laser_cloud->size(),
...
#pragma omp parallel for num_threads(NUM_THREADS)
        ...
        if (pub_image.getNumSubscribers() != 0) {
            ...
            cv::vconcat(image_intensity, image_range, image_visualization);
            cv::cvtColor(image_visualization, image_visualization, CV_GRAY2RGB);
            cv::putText(image_visualization, "Intensity", cv::Point2f(5, 20 + IMAGE_HEIGHT * 1),
                        CV_FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(255, 0, 255), 2);
            cv::putText(image_visualization, "Range", cv::Point2f(5, 20 + IMAGE_HEIGHT * 2), CV_FONT_HERSHEY_SIMPLEX,
        ...
        tf_base_to_lidar.sendTransform(
                tf::StampedTransform(base_to_lidar, cloud_msg->header.stamp, "base_link", "velodyne"));
    }

    void
    pubImage(ros::Publisher *this_pub, const cv::Mat &this_image, std_msgs::Header this_header, string image_format) {
