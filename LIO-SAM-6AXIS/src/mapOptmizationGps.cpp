...
public:
    ...
    ros::Publisher pubVisualLoopConstraintEdge;
...
//    std::queue<nav_msgs::Odometry::ConstPtr> odometryBuf;
    ...
    queue<std_msgs::Float64MultiArray> loopInfoVec;
    vector<pair<int, int>> visualIndexQueue;
    vector<gtsam::Pose3> visualPoseQueue;
    vector<gtsam::noiseModel::Diagonal::shared_ptr> visualNoiseQueue;
    ...
    mapOptimization() {
        ...
        subLoop = nh.subscribe<std_msgs::Float64MultiArray>("lio_sam_6axis/loop_closure_detection",
                                                            1,
        ...
        pubVisualLoopConstraintEdge = nh.advertise<visualization_msgs::MarkerArray>(
                "/lio_sam_6axis/mapping/visual_loop_closure_constraints", 1);
...
//    }
    ...
    void VisualLoopClosureThread(void) {
        ros::Rate rate(5);
        bool status = ros::ok();
        while (status) {
            ros::spinOnce();
            if (loopClosureEnableFlag) {
                PerformVisualLoopClosure();
                visualizeImageLoopClosure();
            }
            rate.sleep();
        }
    }

    void PerformVisualLoopClosure() {
        if (cloudKeyPoses3D->size() < 20)
            return;

        if (loopInfoVec.empty())
            return;
        else if (loopInfoVec.size() > 30) {
            ROS_WARN(
                    "Too many loop clousre candidates to be ICPed is waiting: %d", loopInfoVec.size());
        }

        mtxVisualLoopInfo.lock();
        double loopTimeCur = loopInfoVec.front().data[0];
        double loopTimePre = loopInfoVec.front().data[1];
        loopInfoVec.pop();
        mtxVisualLoopInfo.unlock();

        //  we have check it at the loop detection node
        ...
        for (int i = cloudSize - 1; i >= 0; --i) {
            if (keyframeTimes.at(i) >= loopTimeCur)
                loopKeyCur = i;
            else
                break;
        }
        ...
        int loopKeyPre = 0;
        for (int i = 0; i < cloudSize; ++i) {
            if (keyframeTimes.at(i) <= loopTimePre)
                loopKeyPre = i;
            else
                break;
        }
        ...
        // short time
        //        if (abs(keyframeTimes.at(loopKeyCur) - abs(keyframeTimes.at(loopKeyPre)) < historyKeyframeSearchTimeDiff))
        //            return;
        ...
        if (it != loopIndexContainer.end()) {
            ...
            if (abs(loop_pre_candidate - loopKeyPre) < 5)
                return;
        }
        ...
        float loopScore = 0.2;
        //mtxICP.lock();
        auto relative_pose = doICPPointsRefine(loopKeyPre, loopKeyCur, loopScore, 1);
        //mtxICP.unlock();
        if (!relative_pose.equals(gtsam::Pose3::identity())) {

            gtsam::Vector Vector6(6);
            Vector6 << loopScore, loopScore, loopScore, loopScore, loopScore, loopScore;
            noiseModel::Diagonal::shared_ptr visualConstraintNoise = noiseModel::Diagonal::Variances(Vector6);

            // Add pose constraint
            mtx.lock();
            visualIndexQueue.push_back(make_pair(loopKeyCur, loopKeyPre));
            visualPoseQueue.push_back(relative_pose);
            visualNoiseQueue.push_back(visualConstraintNoise);
            mtx.unlock();

            loopVisualIndexCheckedMap[loopKeyCur] = loopKeyPre;
            ROS_WARN("visual Loop detected %d and %d: %f! ", loopKeyCur, loopKeyPre, loopScore);
        }
    }
    ...
    gtsam::Pose3 doICPPointsRefine(int loopKeyPre, int loopKeyCur, float &score, int type) {
        ...
        // 提取当前关键帧特征点集合，降采样
        loopFindNearKeyframes(cureKeyframeCloud, loopKeyCur, 0);
        //    *cureKeyframeCloud = *Local2global(keyframeLaserRawClouds.at(loopKeyCur), keyframePosesUpdated[loopKeyCur]);
        loopFindNearKeyframes(targetKeyframeCloud, loopKeyPre, historyKeyframeSearchNum);

        if (cureKeyframeCloud->size() < 300 || targetKeyframeCloud->size() < 1000)
            return gtsam::Pose3::identity();;

        //LOG(INFO) << "pcl icp cloud size: " << cureKeyframeCloud->size() << ", " << targetKeyframeCloud->size();
        ...
        Eigen::Affine3f tWrong = pclPointToAffine3f(cloudKeyPoses6D->at(loopKeyCur));
        ...
        gtsam::Pose3 poseFrom = Pose3(Rot3::RzRyRx(roll, pitch, yaw), Point3(x, y, z));
        // 闭环匹配帧的位姿
        gtsam::Pose3 poseTo = pclPointTogtsamPose3(cloudKeyPoses6D->at(loopKeyPre));
    ...
    void visualizeImageLoopClosure() {
        if (loopVisualIndexCheckedMap.empty())
            return;

        visualization_msgs::MarkerArray markerArray;
        // 闭环顶点
        visualization_msgs::Marker markerNode;
        markerNode.header.frame_id = odometryFrame; // camera_init
        markerNode.header.stamp = ros::Time().fromSec(keyframeTimes.back());
        markerNode.action = visualization_msgs::Marker::ADD;
        markerNode.type = visualization_msgs::Marker::SPHERE_LIST;
        markerNode.ns = "visual_loop_nodes";
        markerNode.id = 0;
        markerNode.pose.orientation.w = 1;
        markerNode.scale.x = 0.3;
        ...
        markerEdge.action = visualization_msgs::Marker::ADD;
        markerEdge.type = visualization_msgs::Marker::LINE_LIST;
        markerEdge.ns = "visual_loop_edges";
        markerEdge.id = 1;
        markerEdge.pose.orientation.w = 1;
        markerEdge.scale.x = 0.2;
        markerEdge.color.r = 0.8;
        markerEdge.color.g = 0.2;
        markerEdge.color.b = 0;
        markerEdge.color.a = 1;

        // 遍历闭环
        for (auto it = loopVisualIndexCheckedMap.begin(); it != loopVisualIndexCheckedMap.end(); ++it) {
            int key_cur = it->first;
            int key_pre = it->second;
            geometry_msgs::Point p;
            p.x = cloudKeyPoses6D->at(key_cur).x;
            p.y = cloudKeyPoses6D->at(key_cur).y;
            p.z = cloudKeyPoses6D->at(key_cur).z;
            markerNode.points.push_back(p);
            markerEdge.points.push_back(p);

            p.x = cloudKeyPoses6D->at(key_pre).x;
            p.y = cloudKeyPoses6D->at(key_pre).y;
            p.z = cloudKeyPoses6D->at(key_pre).z;
            markerNode.points.push_back(p);
            markerEdge.points.push_back(p);
        }
        ...
        markerArray.markers.push_back(markerNode);
        markerArray.markers.push_back(markerEdge);
        pubVisualLoopConstraintEdge.publish(markerArray);
    }
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    void addLoopFactor() {
        if (loopIndexQueue.empty() && visualIndexQueue.empty())
            return;
        ...
        // add visual loop
        for (int i = 0; i < (int) visualIndexQueue.size(); ++i) {
            int indexFrom = visualIndexQueue[i].first;
            int indexTo = visualIndexQueue[i].second;
            gtsam::Pose3 poseBetween = visualPoseQueue[i];
            gtsam::noiseModel::Diagonal::shared_ptr noiseBetween = visualNoiseQueue[i];
            gtSAMgraph.add(BetweenFactor<Pose3>(indexFrom, indexTo, poseBetween, noiseBetween));
        }
        visualIndexQueue.clear();
        visualPoseQueue.clear();
        visualNoiseQueue.clear();
...
int main(int argc, char **argv) {
    ...
    std::thread vi_detection{&mapOptimization::VisualLoopClosureThread, &MO};
